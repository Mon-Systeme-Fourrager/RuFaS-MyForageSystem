...
- [TBD](https://github.com/Mon-Systeme-Fourrager/RuFaS-MyForageSystem/pull/TBD) - [Major change] [Animal] [NoInputChange] [OutputChange] Add native stocker/backgrounding module: `AnimalType.BEEF_STOCKER_STEER`/`BEEF_STOCKER_HEIFER` cohort lists in `HerdManager`; `HerdFactory._initialize_beef_stocker_herd` staged via `beef_stocker_animals` ClassVar; `_process_daily_herd_updates` stocker loop fires `AnimalModuleReporter.report_stocker_performance` at exit (SOLD or LIFE_STAGE_CHANGED); `_daily_nutrients_update` stocker phosphorus early-return; `DataValidator.validate_beef_stocker_config` validates weights, max days, target ADG, and diet system (`'pasture'`/`'drylot_forage'`); `Animal._stocker_daily_routines` / `_stocker_life_stage_update` for daily growth and weight/time-based exit. Adds `docs/beef_module/stocker/README.md` with 5 named scope boundaries (BeefGEM deferrals: compensatory gain, limit-feeding, heat stress, pasture growth model, sexed-pen management).
