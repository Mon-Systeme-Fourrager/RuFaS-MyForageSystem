...
- [pending](https://github.com/Mon-Systeme-Fourrager/RuFaS-MyForageSystem/tree/claude/openspec-setup) - [minor change] [Tooling / Docs] [NoInputChange] [NoOutputChange] Configure OpenSpec for the fork: fill `openspec/config.yaml` with the project `context` (fork constraint to avoid upstream-sync merge conflicts, the tooling config-file locations + exact CI commands, and the RuFaS conventions) and per-artifact `rules` (proposal/design/tasks). The optional spec-sync skill (`openspec-sync-specs` / `/opsx:sync`) is already vendored under `.claude/skills`. No custom code.
