...
- [TBD](TBD) - [Major change] [Animal Module] [NoInputChange] [NoOutputChange] Implement beef feedlot module (Steps 1–8 of 8): add beef `Breed` members (AN/HE/SI/CH/LM/BR/XB), `AnimalCombination.FEEDLOT_FINISHING`, `AnimalGroupingScenario.FEEDLOT_ONLY`; add `FEEDLOT_STEER`/`FEEDLOT_HEIFER` to `AnimalType` with `is_feedlot` property and `Sex.STEER`; add NRC 2016 biological constants to `AnimalModuleConstants` (NEm breed/sex multipliers, step-up phase boundaries, mud/cold-stress factors, receiving period); add feedlot config params to `AnimalConfig` and event strings to `animal_constants`; create `BeefNRCRequirementsCalculator` (NRC 2016 Ch. 10–12: NEm, NEg, DMI Eq. 10-1, MP, Ca, P); add feedlot lifecycle to `Animal` (`days_on_feed`, `step_up_phase`, `receiving_stress`, `entry_weight`, `cumulative_dmi`, `_feedlot_daily_routines`, `_feedlot_life_stage_update`); add step-up ration registration (`feedlot_starter/transition/finisher_ration`) to `RationManager` and feedlot constraint set to `RationOptimizer`; add `mud_condition` to `Pen`, `feedlot_animals` list to `HerdManager`, `report_feedlot_performance` to `AnimalModuleReporter`; add 118 feedlot tests including NRC 2016 Box 12-1 MP anchor benchmark (691 g/d, ±0.5 %).
