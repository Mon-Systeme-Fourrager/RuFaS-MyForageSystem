...
- [TBD](https://github.com/Mon-Systeme-Fourrager/RuFaS-MyForageSystem/pull/TBD) - [Major change] [Animal] [NoInputChange] [OutputChange] Add stocker/backgrounding herd management: `HerdFactory._initialize_beef_stocker_herd` populates `HerdFactory.beef_stocker_animals`; `HerdManager` maintains separate `beef_stocker_steers` and `beef_stocker_heifers` cohort lists wired into `animals_by_type`, `_remove_animal_from_current_array`, and `_add_animal_to_new_array`; `_process_daily_herd_updates` iterates both lists and fires `AnimalModuleReporter.report_stocker_performance` at exit for sold or graduated stockers; `_daily_nutrients_update` adds a stocker phosphorus early-return matching the feedlot pattern; `DataValidator.validate_beef_stocker_config` validates entry/exit weights, max days, target ADG, and diet system.
