...
- [TBD](https://github.com/Mon-Systeme-Fourrager/RuFaS-MyForageSystem/pull/TBD) - [minor change] [Tooling] [NoInputChange] [NoOutputChange] Port the agridoc plan/spec-driven dev workflow to RuFaS: scaffold OpenSpec (`opsx/*` commands + skills + `openspec/` via the `@fission-ai/openspec` CLI), add RuFaS-adapted slash commands (`/diagnose`, `/challenge-plan`, `/refine-plan`, `/apply-plan`, `/graphify` — pytest/mypy/flake8, `gh`, `dev-msf`, changelog/protected-input gates), a Python `code-reviewer` agent, and a Dev-workflow section in `CLAUDE.md`.
