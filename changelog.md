...
- [pending](https://github.com/Mon-Systeme-Fourrager/RuFaS-MyForageSystem/tree/claude/docs-style-rules-explained) - [minor change] [Docs] [NoInputChange] [NoOutputChange] Add `docs/style_rules_explained.md`, a plain-language reference for every RuFaS new-code style rule (custom `MSF0xx` checks, enabled Ruff rules, the per-file mypy pass, the AI-reviewed judgement checks, and the deliberately-not-enforced accepted-legacy list), each with a before/after example. Documentation only.
