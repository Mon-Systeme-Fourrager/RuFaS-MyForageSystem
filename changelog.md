...
- [TBD](https://github.com/Mon-Systeme-Fourrager/RuFaS-MyForageSystem/pull/TBD) - [minor change] [Tooling] [NoInputChange] [NoOutputChange] Align the layered `CLAUDE.md` docs and agent skills with the official RuFaS wiki guidelines: add the code-review conventions (NumPy docstrings, edge-case/invalid-input test coverage, ≤200-line PRs, two reviews, "comments discouraged", what/why/how + Test Plan) and the `dev → test → main` branching flow to the root `CLAUDE.md`; add E2E + coverage rules and an anti-`Class.method = MagicMock()` note to `tests/CLAUDE.md`; add wiki reference pointers to `RUFAS/CLAUDE.md`; and add two skills — `rufas-e2e-testing` (run/set-up/update E2E) and `rufas-design-doc` (design-doc-driven process).
