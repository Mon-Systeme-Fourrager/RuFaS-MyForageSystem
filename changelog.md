...
- [40](https://github.com/Mon-Systeme-Fourrager/RuFaS-MyForageSystem/pull/40) - [minor change] [Tooling / Docs] [NoInputChange] [NoOutputChange] Add a path-scoped agent rule `.claude/rules/style.md` (scoped to `RUFAS/**/*.py`, `tests/**/*.py`, `main.py`) stating the recurring RuFaS new-code style conventions — enum-over-string, named constants over magic numbers, None-safe config parsing, NumPy docstrings on every added function, top-level imports, cyclomatic complexity ≤ 10, the test conventions (`mocker.patch.object`, behaviour assertions, `copy.deepcopy` state isolation, `pytest.raises(match=)`), changelog format, and the accepted legacy style not to "fix" (scientific sigils, published coefficients, `dict[str, Any]` boundaries, god-object calculators) — so new code is written compliant up front; reference it from the layered-rules note in `CLAUDE.md`.
