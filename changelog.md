...
- [TBD](https://github.com/Mon-Systeme-Fourrager/RuFaS-MyForageSystem/pull/TBD) - [minor change] [Tooling] [NoInputChange] [NoOutputChange] Refresh the agent docs after the upstream syncs (#28, #29): correct the root `CLAUDE.md` test stack (now `pytest>=9.0.3` + `pytest-lazy-fixtures`, not 7.4.4 + `pytest-lazy-fixture`) and rebuild the graphify dependency-graph report (4328→5840 nodes reflecting the merged model).
