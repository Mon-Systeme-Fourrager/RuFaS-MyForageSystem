...
- [unreleased](https://github.com/Mon-Systeme-Fourrager/RuFaS-MyForageSystem/tree/claude/python-style-linter-custom-azwj62) - [minor change] [Tooling / CI] [NoInputChange] [NoOutputChange] Add a diff-aware MyForageSystem style gate under `tools/msf_style/` that enforces the recurring RuFaS review conventions on new/changed code only (mirroring the mypy count ratchet, so legacy lines are never flagged): Ruff with a narrow dedicated config (`ruff_msf.toml`), project-specific AST/text checks (`MSF001/002` domain magic numbers, `MSF010/011` None-unsafe config parsing, `MSF020` test class-attribute mocking, `MSF030` mutable-classvar returns, `MSF040/041/042` new C901-suppression/scaffolding-ref/bare-type-ignore), changelog format checks (`MSF050/051/052`), and an opt-in per-file `mypy --strict` pass that closes the count-ratchet hole; wire it into a PostToolUse hook (`.claude/hooks/msf-style-check.sh`) and a fork-specific CI workflow (`.github/workflows/msf_style_gate.yml`); add the `msf-style-review` skill for judgement-level conventions a linter cannot decide (enum-over-string, docstring content, DRY, scientific references, test quality, diff-minimalism) and the `msf-style-audit` skill/process to re-mine PR feedback and propose config updates over time; document the rule catalogue and provenance in `tools/msf_style/STYLE_RULES.md`; cover the gate with 42 unit tests under `tests/test_tools/test_msf_style/`. Replace `[unreleased]` with the PR number when the pull request is opened.
