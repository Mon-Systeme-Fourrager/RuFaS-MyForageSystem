...
- [35](https://github.com/Mon-Systeme-Fourrager/RuFaS-MyForageSystem/pull/35) - [minor change] [Animal Module] [NoInputChange] [OutputChange] Beef cow-calf ration management, herd/pen/reporter infrastructure, and weaned calf hand-off (PR-C Steps 6–8): register four beef rations (`beef_lactating_pasture`, `beef_dry_gestating`, `beef_creep_feed`, `beef_replacement_heifer`) in `RationManager` with staged-then-atomic validation (Lesson 3); add `get_beef_seasonal_ration` and `get_beef_creep_feed_supplement` classmethods; add `beef_cow_constraints`/`beef_replacement_constraints` instance attrs and beef dispatch branches to `RationOptimizer._select_constraints` and `handle_failed_constraints`; add `beef_cow_calf_animals` class attr to `HerdFactory`, `_initialize_beef_cow_calf_herd`, and `_beef_cow_calf_update`; add four separate named cohort lists (`beef_cows`, `beef_replacement_heifers`, `beef_calves`, `beef_bulls`) to `HerdManager.__init__` (Lesson 1); wire all four beef groups into `_process_daily_herd_updates` (Lesson 10); add beef-calving `REMAIN` branch in `_perform_daily_routines_for_animals`; add `forage_quality_factor: float = 1.0` to `Pen`; add `AnimalModuleReporter.report_cow_calf_performance` reporting `beef_days_in_pregnancy`, `beef_lactation_day`, `beef_body_condition_score_9`, `beef_times_calved`, `beef_wean_weight_kg` with `None`-guards; add `DataValidator.validate_beef_cow_calf_config` with `math.isfinite` guards on all four fields; add `MAX_BULL_TO_COW_RATIO = 50` to `animal_constants`; extend `_remove_animal_from_current_array` and `_add_animal_to_new_array` for all beef and feedlot type transitions. Note: dynamic month-by-month forage composition lookup (NRC Table 18-2) deferred to a future PR (static ration config used here per Lesson 7 scope boundary).
