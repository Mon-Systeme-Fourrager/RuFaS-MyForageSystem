...
- [pending](https://github.com/Mon-Systeme-Fourrager/RuFaS-MyForageSystem/tree/claude/config-review-bots) - [minor change] [Tooling / CI] [NoInputChange] [NoOutputChange] Configure the review bots to enforce the RuFaS conventions and the fork context: add `.coderabbit.yaml` (path_instructions for `RUFAS/**` and `tests/**` covering magic numbers, None-safe config, enum-over-string, mocking discipline, docstrings and changelog format; Ruff tool enabled; auto-review on `dev-msf`/`dev`) and `.gemini/config.yaml` + `.gemini/styleguide.md` mirroring the same conventions. No custom validation code.
