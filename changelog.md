...
- [TBD](TBD) - [Major change] [Feed Storage] [InputChange] [OutputChange] Add IFSM Preseal phase for Bunker/Pile/Bag silage, finalized lazily per crop; update Bunker/Pile/Bag storage config to replace the `"size"` field with required `width_m`/`height_m`/`diameter_m`/`dry_matter_density_kg_per_m3` fields.
