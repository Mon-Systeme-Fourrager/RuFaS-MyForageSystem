...
- [46](https://github.com/Mon-Systeme-Fourrager/RuFaS-MyForageSystem/pull/46) - [minor change] [Tooling] [NoInputChange] [NoOutputChange] Config-driven fork tooling and style setup (no custom validation code): `.pre-commit-config.yaml` mirroring CI (Black, flake8, mypy); `.coderabbit.yaml` and `.gemini/` configuring the review bots for the RuFaS conventions; `.gemini/settings.json` pointing Gemini at `CLAUDE.md`; `openspec/config.yaml` project context and rules; the path-scoped write-time rule `.claude/rules/style.md` (conventions no linter enforces, with before/after examples, plus the accepted-legacy list); and fork-discipline / onboarding documentation in `CLAUDE.md`, `README.md`, and `docs/onboarding_dev.md`.
