...
- [pending](https://github.com/Mon-Systeme-Fourrager/RuFaS-MyForageSystem/tree/claude/process-docs-onboarding) - [minor change] [Docs] [NoInputChange] [NoOutputChange] Document the development process: add `docs/onboarding_dev.md` (fork discipline to avoid recurrent upstream-sync merge conflicts, local tooling identical to CI with the exact commands and config-file locations, Claude Code, OpenSpec, review bots, and a "where to tweak what" map); add fork-discipline + config-driven-enforcement sections to `CLAUDE.md` and `AGENTS.md`; add a "Working on this fork" section to `README.md`.
