...
- [TBD](https://github.com/Mon-Systeme-Fourrager/RuFaS-MyForageSystem/pull/TBD) - [minor change] [Tests] [NoInputChange] [NoOutputChange] Migrate from the unmaintained `pytest-lazy-fixture` (broke on pytest ≥ 8) to its maintained successor `pytest-lazy-fixtures` (`lf` aliased as `lazy_fixture`, call sites unchanged), and unpin pytest to `9.0.3`. Unblocks future pytest security/compat updates (supersedes the Dependabot bump). Remove the temporary `pytest` major-version ignore in `.github/dependabot.yml` once this lands.
