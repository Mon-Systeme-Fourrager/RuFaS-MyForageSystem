...
- [40](https://github.com/Mon-Systeme-Fourrager/RuFaS-MyForageSystem/pull/40) - [minor change] [Tooling] [NoInputChange] [NoOutputChange] Add a path-scoped agent rule `.claude/rules/style.md` (scoped to `RUFAS/**/*.py`, `tests/**/*.py`, `main.py`) carrying only the RuFaS new-code conventions no linter enforces — fork diff-discipline, enum-over-string, named constants over magic numbers, None-safe config parsing, NumPy docstrings, test/mocking discipline, a short list of review nits, and the accepted legacy style not to "fix" — so new code is written compliant up front.
