...
- [pending](https://github.com/Mon-Systeme-Fourrager/RuFaS-MyForageSystem/tree/claude/tooling-ruff-precommit) - [minor change] [Tooling / CI] [NoInputChange] [NoOutputChange] Add a standalone `ruff.toml` (Ruff config that complements flake8 without overlapping it) and a `.pre-commit-config.yaml` whose local hooks mirror the CI exactly (Black, flake8, mypy reading the same `pyproject`/`.flake8` config files; Ruff reading `ruff.toml`). One Ruff config is consumed by both pre-commit (staged files) and CodeRabbit (PR diff), so no custom validation script is added and legacy code is never flagged.
