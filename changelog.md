...
- [35](https://github.com/Mon-Systeme-Fourrager/RuFaS-MyForageSystem/pull/35) - [minor change] [Animal Module] [NoInputChange] [OutputChange] Beef cow-calf ration management, herd/pen/reporter infrastructure, and weaned calf hand-off (PR-C Steps 6–8): register four beef rations (`beef_lactating_pasture`, `beef_dry_gestating`, `beef_creep_feed`, `beef_replacement_heifer`) in `RationManager` with staged-then-atomic validation; add `get_beef_seasonal_ration` and `get_beef_creep_feed_supplement` classmethods; add `beef_cow_constraints`/`beef_replacement_constraints` instance attrs and beef dispatch branches to `RationOptimizer._select_constraints` and `handle_failed_constraints`; add `beef_cow_calf_animals` class attr to `HerdFactory`, `_initialize_beef_cow_calf_herd`, and `_beef_cow_calf_update`; add four separate named cohort lists (`beef_cows`, `beef_replacement_heifers`, `beef_calves`, `beef_bulls`) to `HerdManager.__init__`; wire all four beef groups into `_process_daily_herd_updates`; add beef-calving `REMAIN` branch in `_perform_daily_routines_for_animals`; add `forage_quality_factor: float = 1.0` to `Pen`; add `AnimalModuleReporter.report_cow_calf_performance` reporting `beef_days_in_pregnancy`, `beef_lactation_day`, `beef_body_condition_score_9`, `beef_times_calved`, `beef_wean_weight_kg` with `None`-guards; add `DataValidator.validate_beef_cow_calf_config` with `math.isfinite` guards on all four fields; add `MAX_BULL_TO_COW_RATIO = 50` to `animal_constants`; extend `_remove_animal_from_current_array` and `_add_animal_to_new_array` for all beef and feedlot type transitions. Note: dynamic month-by-month forage composition lookup (NRC Table 18-2) deferred to a future PR.
- [36](https://github.com/Mon-Systeme-Fourrager/RuFaS-MyForageSystem/pull/36) - [minor change] [Animal Module] [NoInputChange] [NoOutputChange] Add beef cow-calf documentation and integration/benchmark tests (PR-D Steps 9–11): create `docs/beef_module/cow_calf/README.md` documenting five named scope boundaries (simplified conception model, static seasonal ration, no stocker/backgrounding phase, flat forage_quality_factor, grazing energy cost) and integration test scope (Option B: one calving cycle + second breeding season); update root `CLAUDE.md` Beef Module Reference Documents section to include cow-calf reference data pointer; add 730-day integration test (`test_beef_integration.py`) covering one complete calving-weaning cycle with 80 cows + 15 replacement heifers + 3 bulls (Option B scope — second calving outside window without `initial_pregnancy_distribution`); add combined lactating+pregnant benchmark test (`test_beef_integration_benchmarks.py`) verifying simultaneous gestation+lactation energy requirements both positive and pipeline smoke test confirming `BeefCowCalfRequirementsCalculator` is reachable through full dispatch chain. CC-MAINT-1 through CC-DMI-1 unit benchmarks already exist from PR-A (#33).
- [36](https://github.com/Mon-Systeme-Fourrager/RuFaS-MyForageSystem/pull/36) - [minor change] [Animal Module] [NoInputChange] [NoOutputChange] Add beef cow-calf documentation and integration/benchmark tests (PR-D Steps 9–11): create `docs/beef_module/cow_calf/README.md` documenting five named scope boundaries (simplified conception model, static seasonal ration, no stocker/backgrounding phase, flat forage_quality_factor, grazing energy cost) and integration test scope (Option B: one calving cycle + second breeding season); update root `CLAUDE.md` Beef Module Reference Documents section to include cow-calf reference data pointer; add 730-day integration test (`test_beef_integration.py`) covering one complete calving-weaning cycle with 80 cows + 15 replacement heifers + 3 bulls (Option B scope — second calving outside window without `initial_pregnancy_distribution`); add combined lactating+pregnant benchmark test (`test_beef_integration_benchmarks.py`) verifying simultaneous gestation+lactation energy requirements both positive and pipeline smoke test confirming `BeefCowCalfRequirementsCalculator` is reachable through full dispatch chain. CC-MAINT-1 through CC-DMI-1 unit benchmarks already exist from PR-A (#33); fix `herd_factory._initialize_beef_cow_calf_herd` to include `birth_weight` key in all beef animal dicts (previously raised `TypeError: float() argument must be a string or a real number, not 'NoneType'`); add beef guards in `animal.py` `_daily_digestive_system_update` and `daily_growth_update` to skip processing for beef cow-calf types (`DigestiveSystem.process_digestion` and `Growth.evaluate_body_weight_change` only support dairy types).
- [36](https://github.com/Mon-Systeme-Fourrager/RuFaS-MyForageSystem/pull/36) - [minor change] [Animal Module] [NoInputChange] [NoOutputChange] Add beef cow-calf documentation and integration/benchmark tests (PR-D Steps 9–11): create `docs/beef_module/cow_calf/README.md` documenting five named scope boundaries (simplified conception model, static seasonal ration, no stocker/backgrounding phase, flat forage_quality_factor, grazing energy cost) and integration test scope (Option B: one calving cycle + second breeding season); update root `CLAUDE.md` Beef Module Reference Documents section to include cow-calf reference data pointer; add 730-day integration test (`test_beef_integration.py`) covering one complete calving-weaning cycle with 80 cows + 15 replacement heifers + 3 bulls (Option B scope — second calving outside window without `initial_pregnancy_distribution`); add combined lactating+pregnant benchmark test (`test_beef_integration_benchmarks.py`) verifying simultaneous gestation+lactation energy requirements both positive and pipeline smoke test confirming `BeefCowCalfRequirementsCalculator` is reachable through full dispatch chain. CC-MAINT-1 through CC-DMI-1 unit benchmarks already exist from PR-A (#33); fix `herd_factory._initialize_beef_cow_calf_herd` to include `birth_weight` key in all beef animal dicts (previously raised `TypeError: float() argument must be a string or a real number, not 'NoneType'`); add beef guards in `animal.py` `_daily_digestive_system_update` and `daily_growth_update` to skip processing for beef cow-calf types (`DigestiveSystem.process_digestion` and `Growth.evaluate_body_weight_change` only support dairy types).

- [34](https://github.com/Mon-Systeme-Fourrager/RuFaS-MyForageSystem/pull/34) - [minor change] [Animal Module] [NoInputChange] [NoOutputChange] Add beef cow-calf lifecycle and seasonal reproduction (PR-B Steps 4–5): wire `_initialize_beef_cow_calf_animal` into `Animal.__init__` dispatch for all four beef types; add beef safe-default attributes (`days_in_breeding_season`, `calf_at_side`, `dam`, `lactation_day`, `days_since_calving`, `body_condition_score_9`, `times_calved`, `is_open`, `_breeding_weight_event_fired`) with `BeefCowCalfValuesTypedDict` for the persistence path; route `daily_milking_update` and `daily_reproduction_update` to new beef-specific methods; add `_beef_daily_suckling_update`, `_beef_handle_conception_attempt`, `_beef_handle_calving`, `_beef_daily_reproduction_update`, and four life-stage update methods (`_beef_calf_life_stage_update`, `_beef_replacement_heifer_life_stage_update`, `_beef_cow_life_stage_update`, `_beef_bull_life_stage_update`); add `BEEF_CALVING` event constant; add `BeefReproductionProtocol` enum (NATURAL_SERVICE_SEASONAL, AI_SEASONAL, AI_CONTROLLED_BREEDING) and `beef_reproduction_program` attribute to `AnimalConfig`; add `calculate_seasonal_conception_probability` in `beef_reproduction.py` (postpartum anestrus guard, BCS factor, bull ratio factor); add `day_of_year` property alias to `RufasTime`.

- [33](https://github.com/Mon-Systeme-Fourrager/RuFaS-MyForageSystem/pull/33) - [minor change] [Animal Module] [NoInputChange] [NoOutputChange] Add beef cow-calf nutrition requirements calculator (PR-A Step 3): implement `BeefCowCalfRequirementsCalculator` with `CowCalfRequirementsInputs` dataclass supporting all four cow-calf animal types (BEEF_COW, BEEF_HEIFER_REPLACEMENT, BEEF_CALF, BEEF_BULL); NEm uses NRC 2016 Table 19-1 L factor (lactation), COMP (BCS 1–9 scale), and SEX multipliers (Eq.19-1 to 19-4); gestation uses breed-specific CBW with Eq.19-37/19-40/19-41/19-69; lactation uses Wood curve with `math.e` as e^1 (Eq.19-19 to 19-29, AgeFactor 0.85 for first-calf heifers); growth via composition with `BeefNRCRequirementsCalculator` with EQSBW subtracting conceptus weight for pregnant cows (I4); cow DMI via Eq.10-5 (pregnant/non-pregnant intercept variants) and growing-animal DMI via Eq.10-1; minerals use dietary coefficients from Table 19-3 with pregnancy terms active in final 90 days; add `NotImplementedError` guards for `is_beef_cow_calf` types in `NRCRequirementsCalculator`, `NASEMRequirementsCalculator`, and `BeefNRCRequirementsCalculator`; extend `BREED_NEm_MULTIPLIER` to all 31 NRC 2016 Table 19-1 breeds; add breed lookup tables for L factor, CBW, PKYD, milk composition, and 19 named calculator coefficients to `AnimalModuleConstants`.

- [33](https://github.com/Mon-Systeme-Fourrager/RuFaS-MyForageSystem/pull/33) - [minor change] [Animal Module] [NoInputChange] [NoOutputChange] Add beef cow-calf constants and configuration (PR-A Step 2): add 17 biological constants to `AnimalModuleConstants` with NRC 2016/USDA source citations (gestation length, weaning age/weight, birth weight, calf crop rate, stillbirth/survival rates, mature cow weight, DMI fractions, heifer development targets, cull rate, BCS 1–9 scale default, and breeding season length); add 8 cow-calf event strings to `animal_constants` (CALF_WEANED, COW_REBRED, COW_OPEN_AT_PREGNANCY_CHECK, COW_CULLED_AGE, COW_CULLED_OPEN, REPLACEMENT_HEIFER_REACHED_BREEDING_WEIGHT, REPLACEMENT_HEIFER_PROMOTED_TO_COW, STILLBIRTH); add 9 `AnimalConfig` class attributes for cow-calf management parameters with defaults from constants; extend `initialize_animal_config` to read from a `beef_cow_calf` JSON section with safe `.get()` defaults when absent.

- [33](https://github.com/Mon-Systeme-Fourrager/RuFaS-MyForageSystem/pull/33) - [minor change] [Animal Module] [NoInputChange] [NoOutputChange] Add beef cow-calf identity enums (PR-A Step 1): extend `AnimalType` with `BEEF_CALF`, `BEEF_HEIFER_REPLACEMENT`, `BEEF_COW`, `BEEF_BULL` and an `is_beef_cow_calf` property; extend `AnimalCombination` with `BEEF_COW_CALF_PAIR`, `BEEF_GESTATING`, `BEEF_REPLACEMENT`, `BEEF_BULL_BATTERY`; add `AnimalGroupingScenario.BEEF_COW_CALF_HERD` scenario mapping the four new combinations to their animal types.

- [TBD](TBD) - [minor change] [Animal Module] [NoInputChange] [NoOutputChange] Address PR #32 review round 3: call `AnimalModuleReporter.report_feedlot_performance` at both feedlot exit paths (slaughter weight and max days on feed); clamp `_calculate_np_growth` to `max(0.0, …)` to prevent negative protein deposition corrupting Ca/P calculations; refresh `Pen.mud_condition` when `update_animal_combination` is called so reused pens do not retain stale mud state; replace `MagicMock` with a typed `_AnimalStub` in `test_feedlot_enums.py`; move all deferred imports in `test_feedlot_lifecycle.py` to module level.

- [TBD](TBD) - [Major change] [Animal Module] [NoInputChange] [OutputChange] Implement beef feedlot module (Steps 1–8 of 8): add beef `Breed` members (AN/HE/SI/CH/LM/BR/XB), `AnimalCombination.FEEDLOT_FINISHING`, `AnimalGroupingScenario.FEEDLOT_ONLY`; add `FEEDLOT_STEER`/`FEEDLOT_HEIFER` to `AnimalType` with `is_feedlot` property and `Sex.STEER`; add NRC 2016 biological constants to `AnimalModuleConstants` (NEm breed/sex multipliers, step-up phase boundaries, mud/cold-stress factors, receiving period); add feedlot config params to `AnimalConfig` and event strings to `animal_constants`; create `BeefNRCRequirementsCalculator` (NRC 2016 Ch. 10–12: NEm, NEg, DMI Eq. 10-1, MP, Ca, P); add feedlot lifecycle to `Animal` (`days_on_feed`, `step_up_phase`, `receiving_stress`, `entry_weight`, `cumulative_dmi`, `_feedlot_daily_routines`, `_feedlot_life_stage_update`); route `nutrition_requirements.phosphorus` into `nutrients.phosphorus_requirement` for feedlot animals in `_daily_nutrients_update`; add step-up ration registration (`feedlot_starter/transition/finisher_ration`) to `RationManager` and feedlot constraint set to `RationOptimizer`; add `mud_condition` to `Pen`, `feedlot_animals` list to `HerdManager`, `_initialize_feedlot_herd` to `HerdFactory`, `report_feedlot_performance` to `AnimalModuleReporter`, `validate_feedlot_config` to `DataValidator`; add feedlot tests including NRC 2016 Box 12-1 MP anchor benchmark (691 g/d, ±0.5 %).

### Next Version Updates
- [48](https://github.com/Mon-Systeme-Fourrager/RuFaS-MyForageSystem/pull/48) - [minor change] [Tooling] [NoInputChange] [NoOutputChange] Harden the layered `CLAUDE.md` files with conventions learned from the PR #47 review: beef-module extension rules in `RUFAS/biophysical/animal/CLAUDE.md` (dual dispatch-dict registration, extend `animal_groups` instead of parallel loops, `num_*` input-schema naming, merge-then-validate config parsing, calculator `_validate_inputs` on every float field, reporter layering, documented feedlot dead-end limit); sourced-constant + named-clamp rule in `RUFAS/biophysical/CLAUDE.md` with a provenance nuance (numerical guards are documented as such — never fabricate a citation); stricter test conventions in `tests/CLAUDE.md` (`mocker.patch.object` only — string-path patches disallowed, `pytest.approx` on all float comparisons, one home per benchmark scenario with derivation comments verified against the pinned value, mock only production-reachable states, shared per-package fixture builders); a no-speculative-code (YAGNI) rule in the root `CLAUDE.md`; and two repo-wide conventions promoted to `RUFAS/CLAUDE.md` (extend existing structures instead of adding parallel ones; new input blocks mirror the nearest sibling block's key naming), with the animal-module bullets kept as their canonical local instances. Adds a second round from the PR #49 review: disambiguate "protected input fixtures" to "protected input JSON files" across the eight files that used it (root `CLAUDE.md`, `AGENTS.md`, `.claude/rules/protected-inputs.md`, `.claude/agents/code-reviewer.md`, and four `.claude/commands/`) and state in `tests/CLAUDE.md` that test files are freely editable — the overloaded word had led a contributor to believe `test_*.py` was protected; document the `docs/scientific/*.tex` equation-tag registry in `RUFAS/biophysical/CLAUDE.md` (grep all four sources, take max+1 never a gap, edit the `.tex` in the same PR, cite under `References`); warn in `RUFAS/CLAUDE.md` that `dataclasses.replace()` re-runs `__post_init__` and silently resets `field(init=False)` state; and require in `tests/CLAUDE.md` that a mechanism justified in a docstring be covered by a test that fails without it.
- [31](https://github.com/Mon-Systeme-Fourrager/RuFaS-MyForageSystem/pull/31) - [minor change] [Tooling] [NoInputChange] [NoOutputChange] Document the new upstream input directories in the layered `CLAUDE.md`: `input/data/crop_configurations/` in the field subsystem doc, and `input/data/animal_genetics/` + `input/data/animal_health/` in the animal subsystem doc.
- [30](https://github.com/Mon-Systeme-Fourrager/RuFaS-MyForageSystem/pull/30) - [minor change] [Tooling] [NoInputChange] [NoOutputChange] Refresh the agent docs after the upstream syncs (#28, #29): correct the root `CLAUDE.md` test stack (now `pytest>=9.0.3` + `pytest-lazy-fixtures`, not 7.4.4 + `pytest-lazy-fixture`) and rebuild the graphify dependency-graph report (4328→5840 nodes reflecting the merged model).
- [28](https://github.com/Mon-Systeme-Fourrager/RuFaS-MyForageSystem/pull/28) - [Major change] [Whole codebase] [InputChange] [OutputChange] Sync `dev-msf` with upstream `RuminantFarmSystems/RuFaS` `dev` (was 621 commits behind; biophysical model, tests, inputs, output/data-validator updates). Only `README.md` and `pyproject.toml` (pytest pin) conflicted — both resolved to upstream; MSF-specific tooling (`.claude/`, CI `dev-msf` triggers + dependabot exemption + permissions, `.gitattributes`, setuptools `RUFAS*`) preserved. Upstream independently performed the same `pytest-lazy-fixtures` migration and cleaned the class-state test leaks fixed in #26.
- [27](https://github.com/Mon-Systeme-Fourrager/RuFaS-MyForageSystem/pull/27) - [minor change] [Tooling] [NoInputChange] [NoOutputChange] Align the layered `CLAUDE.md` docs and agent skills with the official RuFaS wiki guidelines: add the code-review conventions (NumPy docstrings, edge-case/invalid-input test coverage, ≤200-line PRs, two reviews, "comments discouraged", what/why/how + Test Plan) and the `dev → test → main` branching flow to the root `CLAUDE.md`; add E2E + coverage rules and an anti-`Class.method = MagicMock()` note to `tests/CLAUDE.md`; add wiki reference pointers to `RUFAS/CLAUDE.md`; and add two skills — `rufas-e2e-testing` (run/set-up/update E2E) and `rufas-design-doc` (design-doc-driven process).
- [26](https://github.com/Mon-Systeme-Fourrager/RuFaS-MyForageSystem/pull/26) - [minor change] [Tests] [NoInputChange] [NoOutputChange] Migrate from the unmaintained `pytest-lazy-fixture` (broke on pytest ≥ 8) to its maintained successor `pytest-lazy-fixtures` (`lf` aliased as `lazy_fixture`, call sites unchanged), and unpin pytest to `9.0.3`. Also fix two latent test-isolation bugs pytest 9's collection order exposed (a `LayerData` class-state leak, via an autouse restore fixture; and a non-deterministic `timestamp` in a `test_simulate` assertion). Unblocks future pytest security/compat updates (supersedes the Dependabot bump). Remove the temporary `pytest` major-version ignore in `.github/dependabot.yml` once this lands.
- [25](https://github.com/Mon-Systeme-Fourrager/RuFaS-MyForageSystem/pull/25) - [minor change] [Security] [CI] [NoInputChange] [NoOutputChange] Resolve the open CodeQL alerts on `dev-msf` and harden CI/merge hygiene for a high-volume open-source repo: add least-privilege `permissions:` blocks to the format/lint/test, sphinx, and scientific-PDF workflows; stop logging raw zip codes in `helpful_scripts/emissions_interpolation` (count missing codes instead); exempt `dependabot[bot]` from the changelog gate (real lint/test/type checks still apply); add `.github/dependabot.yml` (grouped pip + github-actions updates, weekly, `dev-msf`); and add `.gitattributes` `changelog.md merge=union` so concurrent changelog additions auto-merge instead of conflicting.
- [21](https://github.com/Mon-Systeme-Fourrager/RuFaS-MyForageSystem/pull/21) - [minor change] [Tooling] [NoInputChange] [NoOutputChange] Port the agridoc plan/spec-driven dev workflow to RuFaS: scaffold OpenSpec (`opsx/*` commands + skills + `openspec/` via the `@fission-ai/openspec` CLI), add RuFaS-adapted slash commands (`/diagnose`, `/challenge-plan`, `/refine-plan`, `/apply-plan`, `/graphify` — pytest/mypy/flake8, `gh`, `dev-msf`, changelog/protected-input gates), a Python `code-reviewer` agent, and a Dev-workflow section in `CLAUDE.md`.
- [20](https://github.com/Mon-Systeme-Fourrager/RuFaS-MyForageSystem/pull/20) - [minor change] [Tooling] [NoInputChange] [NoOutputChange] Vendor the applicable superpowers agent dev-workflow skills into `.claude/skills/` (self-sufficient, no longer dependent on agridoc-builder being an additional working dir): brainstorming, writing/executing-plans, TDD, systematic-debugging, code-review, git-worktrees, etc. (OpenSpec skills are scaffolded separately by the OpenSpec CLI.)
- [19](https://github.com/Mon-Systeme-Fourrager/RuFaS-MyForageSystem/pull/19) - [minor change] [Tooling] [NoInputChange] [NoOutputChange] Add the caveman output-style config (mirroring agridoc-builder, adapted to RuFaS): `CAVEMAN_DEFAULT_MODE=lite` in `.claude/settings.json`, an `AGENTS.md` (caveman-lite style, commit/PR conventions, sensitive-task guard), and a remote-only caveman auto-install block in `.claude/hooks/session-start.sh`.
