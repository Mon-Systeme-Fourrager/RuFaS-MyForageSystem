...
- [TBD](https://github.com/Mon-Systeme-Fourrager/RuFaS-MyForageSystem/pull/TBD) - [minor change] [Security] [CI] [NoInputChange] [NoOutputChange] Resolve the open CodeQL alerts on `dev-msf`: add least-privilege `permissions:` blocks to the format/lint/test, sphinx, and scientific-PDF workflows, and stop logging raw zip codes in `helpful_scripts/emissions_interpolation` (count missing codes instead; clear-text-logging alert).
