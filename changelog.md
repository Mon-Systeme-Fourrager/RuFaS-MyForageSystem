...
- [TBD](https://github.com/Mon-Systeme-Fourrager/RuFaS-MyForageSystem/pull/TBD) - [minor change] [Tooling] [NoInputChange] [NoOutputChange] Vendor the applicable agent dev-workflow skills into `.claude/skills/` (self-sufficient, no longer dependent on agridoc-builder being an additional working dir): the superpowers set (brainstorming, writing/executing-plans, TDD, systematic-debugging, code-review, git-worktrees, etc.) and the OpenSpec skills (propose/explore/apply/archive).
