...
- [TBD](TBD) - [Major change] [Feed Storage] [NoInputChange] [OutputChange] Add IFSM Infiltration phase for Bunker/Pile/Bag silage with RS-conservation ceiling.
- [TBD](TBD) - [Major change] [Feed Storage] [InputChange] [NoOutputChange] Add IFSM Preseal phase for Bunker/Pile/Bag silage, finalized lazily per crop; adds optional `width_m`/`height_m`/`diameter_m`/`dry_matter_density_kg_per_m3` fields to Bunker/Pile/Bag storage config (the old unused `"size"` field is removed) — a storage missing any of them simply skips Preseal, so existing configs are unaffected and none of the shipped example scenarios change output.
- [47](https://github.com/Mon-Systeme-Fourrager/RuFaS-MyForageSystem/pull/47) - [Major change] [Animal] [NoInputChange] [OutputChange] Add native stocker/backgrounding module: `AnimalType.BEEF_STOCKER_STEER`/`BEEF_STOCKER_HEIFER` cohort lists in `HerdManager`; `HerdFactory._initialize_beef_stocker_herd` parses `num_steers`/`num_heifers` input keys; `_process_daily_herd_updates` stocker loop fires `AnimalModuleReporter.report_stocker_performance` at graduation only (no direct-sale path); `_daily_nutrients_update` stocker phosphorus early-return; `DataValidator.validate_beef_stocker_config` validates weights, max days, target ADG, and diet system (`'pasture'`/`'drylot_forage'`); `Animal._stocker_daily_routines` / `_stocker_life_stage_update` for daily growth and weight/time-based exit; `STOCKER_ARRIVAL` event emitted at weaning hand-off; `BEEF_DMI_MIN_NE_CONCENTRATION` constant guards near-zero denominator in DMI Eq.10-5. Adds `docs/beef_module/stocker/README.md` with seven named scope boundaries (BeefGEM deferrals: compensatory gain, limit-feeding, heat stress, pasture growth model, sexed-pen management, post-exit feedlot simulation, direct sale).
