...
\begin{titlepage}
  ...
  {\Large Animal Module\par}
  \vspace{1cm}

  {\large \today\par}
  \vspace{1.5cm} % Add space before the image

  \includegraphics[width=0.25\textwidth]{resources/animal1.png} % Adjust size and filename
...
\section{Herd-level Processes and Management}\label{sec:AnManage}
     
\subsection{Herd Initialization} \label{sec:animalINPUTS}\label{herdinit}
...
\subsection{Pens and Animal Grouping} \label{animgen}
...
id  & The index of the pen  &   &  &  \\

pen\_name  & The name or identifier of a given pen  &   &  &  \\
...
\paragraph{NRC: Calculating Nutrient Requirements of Dairy Cattle} \label{sec:enreq}
...
\subsubsection{Nutrient Requirements of Dairy Cattle (NRC)} \label{sec:NRC}

    \textbf{ Energy Requirements} For details about the variables included in each calculation, please refer to the \hyperref[energykey]{energy requirements key}.
    \vspace{0.2cm}
...
\vspace{0.5cm}
    ...
    \textbf{ Protein Requirements} Reporting of protein requirements is divided into 4 components: maintenance, growth, pregnancy, and lactation (all in metabolizable protein). For details about the variables included in each calculation, please refer to the \hyperref[proteinkey]{protein requirements key}.
    \vspace{0.2cm}
...
\caption{Key variables used in protein requirement calculations.} \label{proteinkey} \\
\toprule
...
NPg & Net protein requirement for growth  & g \\
    ...
    \begin{tcolorbox}[colback=white, colframe=black, title=Important Note]
        Only calcium (Ca) and phosphorus (P) requirements are considered currently. For details about the variables included in each calculation, please refer to the \hyperref[minkey]{mineral requirements key}.
    \end{tcolorbox}
...
\renewcommand{\arraystretch}{1.3}
\begin{longtable}{@{} L{3.5cm} L{3.5cm} L{2.5cm} @{}}
\caption{Key variables used in mineral requirement calculations.} \label{minkey} \\
\toprule
\textbf{Variable} & \textbf{Description} & \textbf{Units} \\
\midrule
\endfirsthead
...
\caption{Key variables used in DMI requirement calculations.} \label{dmikey} \\
\toprule
\textbf{Variable} & \textbf{Description} & \textbf{Units} \\
\midrule
\endfirsthead
...
\paragraph{NASEM: Calculating Nutrient Requirements of Dairy Cattle: Eighth Revised Edition} \label{sec:NASEM}

    \textbf{ Energy Requirements} The maintenance requirement is calculated based on metabolic body weight (MBW, body weight\(^{0.75}\)) adjusted for the tissues associated with pregnancy. For details about the variables included in each calculation, please refer to the \hyperref[nasemenergykey]{energy requirements key}.
    \vspace{0.2cm}
...
\caption{Key variables used in protein requirement calculations.} \label{nasemproteinkey} \\
\toprule
\textbf{Variable} & \textbf{Description} & \textbf{Units} \\
\midrule
\endfirsthead
...
\textbf{ Mineral Requirements} Only calcium and phosphorus requirements are included in the code at present. For details about the variables included in each calculation, please refer to the \hyperref[nasemminkey]{mineral requirements key}.
    \vspace{0.5cm}
...
\renewcommand{\arraystretch}{1.3}
\begin{longtable}{@{} L{3.5cm} L{5.5cm} L{3.5cm} @{}}
\caption{Key variables used in mineral requirement calculations.} \label{nasemminkey} \\
\toprule
\textbf{Variable} & \textbf{Description} & \textbf{Units} \\
\midrule
\endfirsthead
...
\textbf{ Dry Matter Intake Estimation} Dry matter intake estimation is different for lactating cows and dry cows. The sum of the dry matter intake of each feed is assumed to be less than the dry matter intake estimation For details about the variables included in each calculation, please refer to the \hyperref[nasemdmikey]{DMI estimation key}.
    \vspace{0.5cm}
...
\renewcommand{\arraystretch}{1.3}
\begin{longtable}{@{} L{3.5cm} L{5.5cm} L{3.5cm} @{}}
\caption{Key variables used in DMI estimation calculations.} \label{nasemdmikey} \\
\toprule
\textbf{Variable} & \textbf{Description} & \textbf{Units} \\
\midrule
\endfirsthead
...
\paragraph{NASEM: Calculating Amino Acid Requirements} \label{sec:NASEMAA} 
...
\caption{Essential and non-essential amino acids} \label{nasemdmikey} \\
\toprule
...
\caption{Key variables used in calculation of AA.} \label{nasemminkey} \\
\toprule
...
\subsubsection{Energy and Nutrition Supply }\label{sec:nutsupply}
\vspace{0.25cm}

\paragraph{General Nutrient Supply}

\paragraph{Energy Supply}
\vspace{0.2cm}
...
\paragraph{AA Nutrient Supply} \label{aanutrient}
...
\subsubsection{Automated Ration Formulation} \label{autoration}
...
\paragraph{Methodology}
The key difference in the methodology for the user-defined ration methodology is that the feed inclusion rates are bounded by the user_defined_ration_percentage values defined for each feed item (Table RF4). Those percentage values become the minimum and maximum inclusion value for each feed, with +/- the tolerance value (Table RF4) as a fraction of the percentage.

The ration formulation will proceed as described in the \hyperref[autoration]{Automated Methodology} above, but when formulation fails, different outcomes will occur.
...
\section{Animal-level Processes and Management}\label{sec:AnManage}
...
For each animal object, the daily updates are called according to the order described in the \hyperref[sec:ADAY]{Day in the Life for Animal} on a RuFaS Farm. This is:
...
\textbf{Required User Inputs} \label{ANbwinputs}
...
\textbf{Expected Outputs} \label{ANbwoutputs}
...
\subsection{Animal Reproduction} \label{REPRO}
...
\end{itemize}
    ...
    \paragraph{Methodology} \label{repromethod}
...
\subsection{Milk Production} \label{milkprod}
...
\vspace{-0.25cm}
    ...
    \begin{enumerate}
        ...
        At the beginning of the simulation, the herd is assigned distributions for the lactation curve parameters l, m, and n for each parity of cows. It calculates Wood’s lactation curve parameter mean values by parity, adjusted based on the location, production, management practices, +/- reported milk production of the farm being simulated. 
         ...
         These are comprehensively outlines in the \hyperref[lactcurve]{Lactation Curve Parameters Determination} Section.
        \vspace{0.1cm}
...
\(Calculated\ milk\ production = l*t^m *e^{[-nt]}\)
    ...
    \vspace{0.5cm}
        ...
        \begin{tcolorbox}[colback=white, colframe=black, boxrule=0.5pt, arc=3pt, width=\textwidth]
            For more information, refer to the \hyperref[REPRO]{Reproduction} section of this document.
        \end{tcolorbox}
        \vspace{0.2cm}
    ...
    \vspace{0.2cm}
        ...
        \begin{tcolorbox}[colback=white, colframe=black, boxrule=0.5pt, arc=3pt, width=\textwidth]
            For more details on milk changes and reductions, go to the \hyperref[myprodred]{Milk Production and Reduction} portion of this section. . 
        \end{tcolorbox}
...
\subsection{Lactation Curve: Parameters and Definitions} \label{lactcurve}
    ...
    This module is part of the Animal Module and integrates with the \hyperref[milkprod]{Milk Production} calculations. The outputs influence various downstream models, including nutrient requirements, manure excretion, and greenhouse gas emission calculations.
...
\vspace{0.5cm}
    ...
    Equations below are not in the LactationCurve class, but they are useful to understanding the bigger picture of how lactation curve parameter estimates are generated in RuFaS. Please see the \hyperref[milkprod]{Milk Production} section for more details on these equations \parencite{Wood1967}.
    \vspace{0.5cm}
...
\subsection{Culling} \label{herdexit} 
...
\subsection{Manure Excretion}
    ...
    \begin{enumerate}
        \item  \hyperref[ANgeneralmanexcinputs]{General Animal Inputs}
        \item \hyperref[ANcowspecinputs]{Cow Specific Inputs} 
        \item \hyperref[ANPinputs]{Phosphorus Specific Inputs}
    \end{enumerate}


% Begin longtable
\textbf{General Inputs} Manure Excretion \label{ANgeneralmanexcinputs}
...
\textbf{\large Cow Specific Inputs – Manure Excretion} \label{ANcowspecinputs}
...
\textbf{\large Phosphorus Excretion Inputs – Manure Excretion} \label{ANPinputs}
...
\caption{Expected outputs from this section.} \label{ANoutputes} \\
\toprule
...
Because the equations that predict total manure excreted and total solids excreted rely on different inputs, in some extreme cases, the total mass of manure excreted can be predicted to be very close to the predicted total solids excreted which is unrealistically dry. For this reason, we set a minimum manure dry matter content of 15\% for heifers and 12\% for dry cows. If the mass predicted by equation \textbf{[AN.EXC.11]} is not greater than (total solids/minimum dry matter) then, 
        \vspace{0.5cm}
...
Total manure excretion (kg/d): Amount of feces and urine excreted daily by dry cows (kg/d).
    ...
    \end{center}
        \vspace{0.5cm} 
            ...
            \vspace{0.5cm}
Because the equations that predict total manure excreted and total solids excreted rely on different inputs, in some extreme cases, the total mass of manure excreted can be predicted to be very close to the predicted total solids excreted which is unrealistically dry. For this reason, we set a minimum manure dry matter content of 15\% for heifers and 12\% for dry cows. If the mass predicted by equation AN.EXC.11 is not greater than (total solids/minimum dry matter) then, 
...
methane\_mitigation\_additive\_amount & Dosage per kg of DMI & 3-NOP: 70 (40–100); Monensin: 24 (20–36) mg/kg DMI \\
\end{longtable}
