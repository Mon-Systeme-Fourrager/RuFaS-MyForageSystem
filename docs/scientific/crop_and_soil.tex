...
\addbibresource{resources/crop_and_soil.bib}  % Your .bib file name
\usepackage{titlesec}
\usepackage{etoolbox}
...
% Roman for \section, Alpha for \section, alpha for \subsection
...
\titleformat{\subsection}
  {\Large\bfseries}   % Subsection: medium
  {\thesubsection}
  {1em}
  {}
...
\begin{titlepage}
  ...
  {\Large Soil and Crop\par}
  \vspace{1cm}

  {\large \today\par}
  \vspace{1.5cm} % Add space before the image

  \includegraphics[width=0.25\textwidth]{resources/crop.png} % Adjust size and filename

  \vfill

  {\small v.1; Updated 2025\par}
  \vspace*{1cm}
\end{titlepage}
...
The first step in execution of a fertilizer application is to calculate the total mass of fertilizer required to meet the minimum N, P, and K application masses requested for each fertilizer given the specified nutrient composition of the fertilizer mix using the following two methods:
    ...
    \begin{center}
        \(fert\_massnitrogen=\frac{mass\_requested\_nitrogen}{fert\_nitrogen\_frac
fert\_mass}\) \\ 
        \(phosphorus=\frac{mass_requested_phosphorus}{fert_phosphorus_frac}\) \\
        \(fert\_masspotassium=\frac{mass\_requested\_potassium}{fert\_potssium\_frac}\)
    \end{center}
        \vspace{0.5cm} 
    ...
    \begin{center}
        \(applied\_mass = maxfert\_mass_{nitrogen},\ fert\_mass_{phosphorus}\ fert\_mass_{potassium} \)
    \end{center}
        \vspace{0.5cm} 
...
\begin{itemize}
    ...
    \item fert\_massnitrogen (kg) is the mass nitrogen requested.
    \item fert\_massphosphorus (kg) is the mass of phosphorus requested.
    \item fert\_masspotassium (kg) is the mass of potassium requested.
...
\section{Soil Management} \label{soilmgt}
\subsection{Introduction}
\vspace{0.2cm}
...
\textbf{ \large Soil Temperature} (Implemented in soil\_temp.py)
...
\(dd_{\max}=1000 + \frac{2500 BD}{BD + 686\exp(-5.63 BD)}\)
    ...
    \begin{center}
\(dd = dd_{\max} \times \exp\left\{ \ln \left(\frac{500}{dd_{\max}}\right) \times \left[\frac{(1-\text{scale})}{(1+\text{scale})}\right]^2 \right\}\)
    \end{center}
        \vspace{0.5cm} 
...
The impact of plant and snow cover is quantified as a weighting factor, bcv, given by the maximum of the following two equations:
    ...
    \begin{center}
\(bcv= \frac{plant\_cover}{plant\_cove+exp(7.563-0.0001297) * plant\_cove}\)
    \end{center}
        \vspace{0.5cm} 
...
\(bcv=\frac{SNOW}{SNOW+exp(6.055-0.3002) * SNOW}\)
    \end{center}
        ...
        \begin{itemize}
            ...
            \item plant\_cove = Total aboveground plant biomass and residue on the current day (kg per hectare)
...
Evapotranspiration (ET) is one of the most important hydrological processes of soil water balance in the soil and crop module. It is the sum of all processes by which water moves from the land surface to the atmosphere via soil evaporation, plant canopy transpiration, and sublimation. The Hargreaves method requires air temperature only. RuFaS adapts the Hargreaves method \parencite{hargreaves1985} to estimate ET which only requires temperature as an input.
\vspace{0.2cm}
...
\paragraph{Potential Evapotranspiration  (Implemented in field.py)}        
 ...
 \( LHV * ET_{max}=0.0023 * H_0 * (T_{max} - T_{min})^{0.5} * (T_{avg} + 17.8) \)
    \end{center}
        ...
        \begin{itemize}
            ...
            \item ET\textsubscript{max} is taken as the maximum between 0.001 and the value calculated by solving for ET\textsubscript{max} in the equation above.
        \end{itemize}
\vspace{0.2cm}
...
\paragraph{Maximum crop transpiration (implemented in water\_dynamics.py)}        
...
\paragraph{Sublimation and soil evaporation (implemented in field.py)}        
...
Potential evaporation is adjusted based on trans\textsubscript{max} as: 
    ...
    \begin{center}
 \(evap_{max}=min\ \{      evapmaxevapmaxETmaxevapmax+transmax \} \)
...
\paragraph{Partition evaporation within soil profile (implemented in evaporation.py)}     
\vspace{0.5cm}
...
evaporative demand for the layer is reduced according to the following equation: evaporation.py
\vspace{0.5cm}

    \begin{center}
 \(evap_{ly}=evap_{ly} * exp[2.5 * \frac{SW-FC}{FC-WP}]\ \ \ \ \ \ \ if\ SW<FC\)
    \end{center}
        \vspace{0.5cm} 
...
\[
evap_{ly}=min = min\left\{ 0.8 * SW-WP,\ evap_{ly} \right\}
\]
...
Percolation is the process of water movement downward through the soil profile. When the soil layer water is less than or equal to field capacity no percolation occurs. Percolation is not calculated for frozen layers.  \vspace{0.5cm}
...
\textbf{\large {Soil Erosion} (Implemented as  soil\_erosion.py)}
\vspace{0.2cm}

\textbf{\textit{Base Equation, Rainfall Intensity, and Peak Runoff}}
\vspace{0.2cm}
...
Soil erosion is a geological process whereby surface soil is removed by water/wind and  anthropogenically. In RuFaS, erosion is modeled from individual crop fields using the Modified Universal Soil Loss Equations (MUSLE, SWAT 2009 documentation). The MUSLE input variables include rainfall (rainfall amount and intensity), soil erodibility factor, soil cover/management, best management practices and the topographic factor. 
 ...
 \subsection{Base Equation, Rainfall Intensity, and Peak Runoff}

    \begin{center}
 \(sed=11.8 *( Qsurf*qpeak*field\_size)^{0.56} * K * C * P * LS\)
    \end{center}
        \vspace{0.5cm} 
...
Soil erodibility factor K is calculated as:
 ...
 \(K=f_{csand} * f_{cl-si} * f_{orgc} * f_{sand}\)
    \end{center}
        ...
        \begin{itemize}
            \item Fcsand	=	gives low factors for soils with high sand contents and high values for soils with little sand
            \item Fcl-si	=	gives low factors for soils with high clay to silt ratios
            \item Forgc	=	reduces soil erodibility for soils with high organic carbon content
            \item Fsand	=	reduces soil erodibility for soils with high sand contents
        \end{itemize}
    \vspace{0.5cm}
...
\vspace{0.2cm}
    ...
    \begin{center}
 \(mass (kgN ha^{-1})=\frac{(nutrient conc (mg kg^{-1}) * BD * depth * field size)}{field size}\)
    \end{center}
        \vspace{0.5cm} 
     ...
     \begin{center}
 \(mass (kgN ha^{-1})=mg kg^{-1} * BD * depth10\)
    \end{center}
        \vspace{0.5cm} 
...
\textit{Nitrification and Volatilization Regulation}
...
The total amount of NH\textsubscript{4} (kg N ha\textsuperscript{-1}) converted through nitrification and volatilization is given by:
\vspace{0.5cm}
...
\vspace{0.5cm}
 ...
 \(NConc= \frac{100 * N(kg N/ha)}{bulk\_density * depth}\)
...
\(denitrified\_nitrates_{layer[i]} = nitrate\_content_{layer[i]} * max( min( 1.0, 1 - exp(-denitrification\_rate\_coefficient * temp\_factor_{layer[i]} * organic\_carbon\_percent_{layer[i]})),\)
    ...
    \vspace{0.2cm}
        ...
        \begin{itemize}
            ...
            \item nutrient\_cycling\_soil\_water\_factorlayer[i] is the nutrient cycling water factor for the layer i
            \item soil\_water\_threshold is the nutrient cycling water factor threshold value for the layer. According to Wen et al. (2024) the value for the SWthr is typically set to 1.0 meaning that denitrification is allowed to occur when water content is above field capacity. 
...
\(Partitioning\_factor = max((min\ (NO3\_effect, C\_effect) * moisture\_effect *pH\_effect), 0.0)\)
   ...
   \begin{center}
\(NO3\_effect=(1.0-[0.5+\{atan(pi*0.01*\frac{(NO3\_content-190)}{pi})\}] *25)\)
  \end{center}
    \vspace{0.5cm}
   ...
   \begin{center}
\(Ceffect=13 +\{\frac{[30.78*atan[pi*0.07*(Cresp*1000-13)]}{pi} \}\)
  \end{center}
    \vspace{0.5cm}
...
\(pH\_effect=\frac{1}{(1470*e^{(-1.1*pH)} )} \)
    ...
    \vspace{0.5cm}
        ...
        \begin{itemize}
            ...
            \item atan= the arc tangents measured in radians of x (values are between -pi/2 and pi/2)
            \item NO3\_content is the amount of nitrate in the layer (ug N/g soil)
            \item Cresp is carbon respiration from the soil layer (kg ha\textsuperscript{-1})
...
\vspace{0.5cm}
    ...
    \begin{center}
\(nitrous\_oxide\_nitrogen = (\frac{denitrified\_nitrates}{(1 + partition_factor)})* 0.001\)
    \end{center}
        \vspace{0.5cm} 
            ...
            \vspace{0.5cm}
    \begin{center}
\(N_2O(kg)=(\frac{nitrates}{(1 + partition_factor)})* 0.001 \)
...
\vspace{0.5cm}
    ...
    \begin{center}
\(Nminact=Minrate * (temp\_factor * water\_factor)^{0.5} * activeN\)
    ...
    \end{center}
        ...
        \begin{itemize}
            \item Minrate is the rate coefficient for mineralization of the humus active organic nitrogen
            \item TempFac is the nutrient cycling temperature factor for the layer
            \item WaterFac is the nutrient cycling water factor for the layer
...
\vspace{0.5cm}
    ...
    \begin{center}
\(C:N=\frac{structural\ litter + metabolic\ litter}{FreshN + NO3}\ \ \ \ \ \ C:N=0\ if\ FreshN + NO3 \leq 0\)
\vspace{0.1cm}

\(C:N=\frac{structural\ litter + metabolic\ litter}{FreshOrganicP + Psol}\ \ \ \ if FreshOrganicP+LabileP \leq 0\)
    ...
    \end{center}
        ...
        \begin{itemize}
            \item res is the above ground soil residue (kg ha\textsuperscript{-1})
            \item C is the sum of structural and metabolic litter in the soil layer (kg ha\textsuperscript{-1})
            \item freshN is the N in the fresh organic pool in the layer (kg ha\textsuperscript{-1})
            \item NO\textsubscript{3} is the amount of nitrate in the layer
            \item FreshOrganicP is the P in the fresh organic  pool in the layer (kg P ha\textsuperscript{-1})
            \item LabileP is the amount of P in the solution in the layer (kg P ha\textsuperscript{-1})
        \end{itemize}
        \vspace{0.5cm}
...
\(ResComp = 1.0\)
    ...
    \end{center}
        ...
        \begin{itemize}
            \item ResComp is the nutrient cycling residue composition factor
            \item MinCoeff is the rate coefficient for mineralization of the residue fresh organic nutrients (user-defined, default - 0.05)
...
Mineralization of the residue fresh organic N (FreshMin) (kg N ha\textsuperscript{-1}) is calculated as:
\vspace{0.5cm}
...
\(ResComp = 1.0\)
    ...
    \end{center}
        ...
        \begin{itemize}
            \item Decay is the residue decay rate constant, FreshN is the N in the fresh organic pool in the layer (kg N ha\textsuperscript{-1}). 20\% of the N decomposed from the FreshMin is then transferred to the active organic N pool, and 80\% is transferred to the NO\textsubscript{3}  pool. N in the fresh organic pool is set to 0 in all layers except the top layer. In the top layer, 0.15\% of residue is decomposed into fresh organic N pools. 
        \end{itemize}
        \vspace{0.5cm}

\textit{Mineralization (Implemented in humus\_mineralization.py)}
\vspace{0.2cm}
...
\vspace{0.5cm}
    ...
    \begin{center}
\(amount\_transferred=0.00001 * active\_organic\_nitrogen  *[\frac{1}{FracN}-1]-stable\_organic\_nitrogen \)
    ...
    \end{center}
        ...
        \begin{itemize}
            \item 10\textsuperscript{-5} is the humus mineralization rate constant
...
Before the soil P pools are manipulated, they are converted into a kg ha\textsuperscript{-1} basis:
\paragraph{Soluble Phosphorus (Implemented in soluble\_phosphorus.py)}

Calculates the P transferred via hydrological processes in the soil based on equations from the APLE and SurPhos models \parencite{vadas2007}. Soluble P can be lost from the soil as runoff in the first layer and leached vertically down through the soil profile.
\vspace{0.2cm}

\textit{Soluble Phosphorus Runoff}
        \vspace{0.5cm}
            
    \begin{center}
\(adjusted\ DRP_{runoff}=min(labileP,\ soilP * extraction\ coefficient * runoff * 10^{-6})\)
    ...
    \end{center}
        ...
        \begin{itemize}
            ...
            \item Runoff is the amount of water lost from the surface layer (L ha\textsuperscript{-1}) soilP is:

        \end{itemize}
        \vspace{0.5cm}
            
    \begin{center}
\(soilP=\frac{labile\_phosphorus * 10}{bulk\ density * thickness_{cm}}\)
...
\textit{Soluble Phosphorus Leaching}
        \vspace{0.5cm}
...
Phosphorus leaching is predicted by the clay isotherm function in the APLE documentation \parencite{vadas2007}.
    ...
    \begin{center}
\(isotherm\ slope=173.51 * clay\ fraction+8.48\)
    ...
    \end{center}
        ...
        \begin{itemize}
            ...
            \item Clay fraction is the fractional clay content of a soil layer (unitless)
        \end{itemize}
        \vspace{0.5cm}
...
\(isotherm\ inter=4.726 * isotherm\ slope-8.97\)
    ...
    \end{center}
        ...
        \begin{itemize}
            \item Isotherm inter is the intercept of P sorption isotherm (unitless)
            \item Isotherm slope is the slope of the P sorption isotherm (unitless) 
...
The maximum leachate amount specified in APLE is 20 (mg L\textsuperscript{-1}), specified in the paragraph beneath equation [16] and is applied here as the min term. Leached P is added to the soil layer immediately below the layer for which it is calculated.
\vspace{0.2cm}
...
\(sorp\ \% = rainday_{fact}\)
    ...
    \end{center}
        ...
        \begin{itemize}
            \item raindayfact is the rain day factor calibrated as described above
        \end{itemize}
        \vspace{0.2cm}
...
\(PDfact=0.034 * exp(3.4 *(\frac{runoff}{rainfall}))\)
    ...
    \end{center}
        ...
        \begin{itemize}
            ...
            \item fraction of P released	is the fraction of P solubilized during the current rain event (unitless).
...
\parencite{vadas2007}
Leached fertilizer P is added to soil labileP for each layer as a function of depth:
        \vspace{0.5cm}
...
\(labileP=labileP+(fert_{leach}-fert_{runoff}) * DF\)
    ...
    \end{center}
        ...
        \begin{itemize}
            \item decreasing fraction of fert P to be leached from one layer to the next
        \end{itemize}
        \vspace{0.5cm}
...
\(field\ size_{in\ sq\ mm}= field\ size\ *10000000000\)
    ...
    \begin{center}
\(MIO = MOP_{leach\ in\ mg} *(\frac{1}{rainfall})*(\frac{1}{field\ size\ in\ mm^2})*(\frac{1}{mm^3\ to\ L})*PDfact\)
...
\(labileP=labileP*DF\ (used\ when\ manure\ is\ applied\ in\ the\ field)\)
\vspace{0.1cm}
...
\textit{Manure Decomposition (implemented in manure\_pool.py)}
        \vspace{0.5cm}
        
    \begin{center}
\(wet_{rate} = -0.3 × moisture + 0.27\)
\vspace{0.1cm}
...
\(dry_{rate} = (-0.5 *(\frac{manure_{mass}}{mass}) + 0.075) * T_{fac}\)
    \end{center}
        ...
        \begin{itemize}
            ...
            \item moisture is the manure water content (unitless)
        \end{itemize}
        \vspace{0.5cm}
...
\(P_{dcom} = min(P,\ max(0.0,\ Ppools * 0.01 * min(TFA,\ moisture))) \)
    \end{center}
        ...
        \begin{itemize}
            \item P\textsubscript{pools} represents each of the tracked P pools (SIP, SOP, WOP)
        \end{itemize}
        \vspace{0.5cm}
...
\(labileP=labileP+DP * DF\)
    \end{center}
        ...
        \begin{itemize}
            \item DF is the Depth Factor, a terminating geometric sequence modeling the decreasing 		fraction of decomposed P to be leached from one layer to the next
        \end{itemize}
        \vspace{0.5cm}


\textit{Runoff (implemented in manure\_pool.py)}
\vspace{0.2cm}
...
Manure and soil P in the first layer are susceptible to runoff:
    ...
    \begin{center}
\(soilP is\ the\ labileP \div bulk\ density \div thickness_{cm} \div 0.1 \)
    \end{center}
        \vspace{0.5cm} 
...
\(TIP_{runoff} = MIP_{runoff} + MDRP_{runoff}+ fertP_{runoff}\)
    \end{center}
        \vspace{0.5cm} 
            ...
            {\raggedleft \textbf{[SC.PSL.57]}\par}
            \vspace{0.5cm}
...
The amount of P that mineralizes into water-extractable inorganic P on the current day from the given pool.
    ...
    \begin{center}
\(mineralization\ rate= rate*min(temp_{factor},\ moisture_{factor})\)
    \end{center}
        \vspace{0.5cm} 
        ...
        \begin{itemize}
            \item rate is the rate factor for the type of P being mineralized (unitless).
            \item temp\textsubscript{factor} is the temperature factor on the current day (unitless).
            \item moisture\textsubscript{factor}  is the moisture factor of the given manure pool on the current day (unitless).
            \item P\textsubscript{amount}  is the amount of P that is mineralized from that pool that is passed (kg)
        \end{itemize}
        \vspace{0.5cm}
...
\(PSP_{act}= max(0.05,\ min(0.7,( \frac{(meanPSP*364)+currentPSP)}{365}))\)
    \end{center}
        ...
        \begin{itemize}
            \item PSP\textsubscript{act}	 is the actual P sorption parameter (unitless)
...
\(stable_{pflow} = min(stablePflow,amounttotransfer)\ \ \ \ \ \  if\ amounttotransfer > 0\)
\vspace{0.1cm}

\(stable_{pflow} = (-1*amounttotransfer)\ \ \ \ \ \  if\ amounttotransfer > activeP\)
    \end{center}
        \vspace{0.5cm} 
    
            {\raggedleft \textbf{[SC.PMN.9]}\par}
            \vspace{0.5cm}
...
\textit{Sediment P Transport in Runoff (implemented in soil\_erosion.py)}
\vspace{0.2cm}

The following calculation of sediment P transport in runoff is based on SWAT \parencite{neitsch2011swat}. However, there are differences in how these equations are applied in RuFaS compared to SWAT, and it is recommended that the equations as applied in Rufas be calibrated using observational data. 
\vspace{0.2cm}

Organic and inorganic P attached to soil particles are lost to runoff during erosion. The amount of P lost with sediment is calculated with a loading function \parencite{mcelroy1977nitrogen} that was modified by \parencite{williams1978optimal}:


\subsection{Carbon Cycling}

\textit{Decomposition Factors (implemented in decomp\_factors.py)}
\vspace{0.2cm}
...
\textit{Residue Partitioning (implemented in residue\_partitioning.py)}
\vspace{0.2cm}
...
\textit{Above Ground: Lignin, Metabolic C, Structural C}
\vspace{0.2cm}
...
Above ground residue is further partitioned into metabolic and structural components based on the lignin composition of the residue. 
    ...
    \begin{center}
\(AG_{lignin\ res\ \%}=0.12 * rainfall * 0.1\)
    \end{center}
        \vspace{0.5cm} 
        ...
        \begin{itemize}
            \item AG\textsubscript{lignin res \%} = lignin composition of above ground plant residue
    ...
    \begin{center}
\(AG_{L:N}=\frac{\frac{AG_{lignin}\ res\ \%}{100}}{fr_N}\ \ \ \ \ \ \) if fr\textsubscript{N}!=0 
\vspace{0.3cm}
...
\(AG_{met\ \%}=0.85-0.18 * AG_{L:N}\)
    \end{center}
        ...
        \begin{itemize}
            \item AG\textsubscript{met \%} = above ground residue that is metabolic (\%)
        \end{itemize}
        \vspace{0.5cm}
...
Above ground metabolic C is decomposed into active C and incorporated into the below ground pool via tillage.
    ...
    \begin{center}
\(AG_{met}:C_{active}=AG_{met_{active\ decomp}} * M_d *T_d * AG_{met}\)
    \end{center}
        \vspace{0.5cm} 
        ...
        \begin{itemize}
            \item AG\textsubscript{met}:C\textsubscript{active}	=above ground metabolic C decomposed to active carbon (kg ha\textsuperscript{-1}) 
    ...
    \begin{center}
\(AG_{met}:BG_{met}=AG_{met} * tillage_\%	\)
    \end{center}
        \vspace{0.5cm} 
        ...
        \begin{itemize}
            ...
            \item tillage = metabolic C incorporated into soil during tillage (unitless)
        \end{itemize}
        \vspace{0.5cm}
...
Any dry matter residue at harvest that is not added to the above ground metabolic C pool is added to the above ground structural C pool:
    ...
    \begin{center}
\(AG_{struct}+=AG_{DM\ res} * (1-AG_{met_\%})	\)
    \end{center}
        \vspace{0.5cm} 
...
\(AG_{struct\ decomp}=K1 * e^{-3} * 1-AG_{met\ \%}	\)
    ...
    \begin{center}
\(AG_{struct}:C_{active}=AG_{struct\ decomp} * M_d * T_d * AG_{struct}\)
\vspace{0.1cm}

\(AG_{struct}:C_{slow}=AG_{struct_{decomp}} * M_d * T_d * AG_{struct}\)
    \end{center}
        \vspace{0.5cm} 
    ...
    \begin{center}
\(AG_{struct}:BG_{struct}=AG_{struct} * tillage_\%\)
    \end{center}
        \vspace{0.5cm} 
...
Similarly, the residue below ground is partitioned into metabolic and structural C pools based on the lignin content of the residue. In the case of tillage, some residue is incorporated from the above ground pool.
    ...
    \begin{center}
\(BG_{DM\ res}=AG_{DM\ res} * tillage_\%\)
    \end{center}
        \vspace{0.5cm} 
...
\(DM_{lignin\ res\ \%}=\frac{BG_{DM\ res}}{BG_{DM\ res}+BG_{bio}}\)
    \end{center}
        ...
        \begin{itemize}
            \item DM\textsubscript{lignin res \%} = weighted fractional amount of lignin in residue dry matter (\%)
...
\(BG_{lignin\ res\ \%}=DM_{lignin\ res\ \%}-0.15 * rainfall * 0.01	\)
    \end{center}
        ...
        \begin{itemize}
            \item BG\textsubscript{lignin res}=	below ground residue comprised of lignin (\%)
        \end{itemize}
        \vspace{0.5cm}
...
The below ground ratio of lignin to N will then be a weighted composition of the lignin to N ratio in the below ground and the incorporated biomass.
    ...
    \begin{center}
\(BG_{L:N}=AG_{L:N} * DM_{ligning\ res\ \%}+ (\frac{\frac{BG_{lignin\ res\ \%}}{100}}{fr_N}) * (1-DM_{lignin\ res\ \%})\)
    \end{center}
        \vspace{0.5cm} 
...
\(BG_{met\ \%}=0.85-0.18 * BG_{L:N}	\)
    \end{center}
        ...
        \begin{itemize}
            \item BG\textsubscript{met\%}=	below ground residue that is metabolic (\%)
        \end{itemize}
        \vspace{0.5cm}
...
\(BG_{met}:C_{active}=BG_{met_{active\ decomp}} * M_d * T_d * BG_{met}\)
    \end{center}
        ...
        \begin{itemize}
            \item BG\textsubscript{met}:C\textsubscript{active} = below ground metabolic C decomposed into active C (kg ha\textsuperscript{-1})
...
Below ground structural C is decomposed into active and slow C.
    ...
    \begin{center}
\(BG_{struct}:C_{active}=BG_{struct\ decomp} * M_d * T_d * BG_{struct}\)
\vspace{0.1cm}

\(BG_{struct}:C_{slow}=BG_{struct_{decomp}} * M_d * T_d * BG_{struct}\)
    ...
    \end{center}
        ...
        \begin{itemize}
            \item BG\textsubscript{struct}:C\textsubscript{active} = below ground structural C decomposed into active C (kg ha\textsuperscript{-1})
            \item BG\textsubscript{struct}:C\textsubscript{slow} = below ground structural C decomposed into slow C (kg ha\textsuperscript{-1})
...
\textit{Pool and Gas Partitioning (implemented in pool\_gas\_partitioning.py)}
\vspace{0.2cm}

There is an imperfect transfer between the metabolic and structural C pools to the active and slow C pools. The amount of C lost to CO2 during the decomposition process is modeled as:
        \vspace{0.5cm}

    \begin{center}
\(AG_{met}:C_{active\ loss}=AG_{met}:C_{active} * \%CO2_{met:active}\)
\vspace{0.1cm}

\(AG_{met}:C_{active\ act}=AG_{met}:C_{active} * (1-\% CO2_{met:active})\)
    ...
    \end{center}
        ...
        \begin{itemize}
            \item \%CO2\textsubscript{met:active} = rate of C dioxide loss during transformation of metabolic to active C (0.55, \cite{parton1987analysis})
            \item AG\textsubscript{met}:C\textsubscript{active loss} = above ground metabolic C being lost as carbon dioxide during decomposition into active C (kg ha\textsuperscript{-1})
            \item AG\textsubscript{met}:C\textsubscript{active act} 	=above ground metabolic C decomposed to active carbon after accounting for C dioxide loss (kg ha\textsuperscript{-1})
        \end{itemize}
        \vspace{0.5cm}
...
\(AG_{struct}:C_{active\ act}=AG_{struct}:C_{active} * (1-\%CO2_{struct:active})\)
    ...
    \end{center}
        ...
        \begin{itemize}
            \item \%CO2\textsubscript{struct active} = rate of CO\textsubscript{2} loss during transformation of structural to	active C (0.45, \parencite{parton1987analysis})

            \item AG\textsubscript{struct}:C\textsubscript{active act} = above ground structural C decomposed to active C after accounting for CO\textsubscript{2} loss C (kg ha\textsuperscript{-1})
            \item AG\textsubscript{struct}:C\textsubscript{active loss} = above ground structural C being lost as CO2 during decomposition into active C (kg ha\textsuperscript{-1})
        \end{itemize}
        \vspace{0.5cm}
...
\(AG_{struct}:C_{slow\ act}=AG_{struct}:C_{slow} * (1-\%CO2_{struct:slow})\)
    ...
    \end{center}
        ...
        \begin{itemize}
            \item AG\textsubscript{struct}:C\textsubscript{slow loss} = ecomposing above ground structural C being lost as CO2 during decomposition into slow C (kg ha\textsuperscript{-1})
            \item AG\textsubscript{struct}:C\textsubscript{slow act}  = above ground structural C decomposed to slow C after accounting for CO2 loss (kg ha\textsuperscript{-1})
            \item \%CO2\textsubscript{struct:slow} = rate of CO2 loss during transformation of structural to slow C (0.3, Partonet al. 1987)
        \end{itemize}
        \vspace{0.5cm}
...
\(BG_{met}:C_{active\ act}=BG_{met}:C_{active} * (1-\%CO2_{met:active})\)
    ...
    \end{center}
        ...
        \begin{itemize}
            \item BG\textsubscript{met}:C\textsubscript{active loss} = below ground metabolic C being lost as CO\textsubscript{2} during decomposition into active C (kg ha\textsuperscript{-1})
            \item BG\textsubscript{met}:C\textsubscript{active act} = below ground metabolic C decomposed to active C after accounting for CO\textsubscript{2} loss (kg ha\textsuperscript{-1})
        \end{itemize}
        \vspace{0.5cm}
...
\(BG_{struct}:C_{active\ act}=BG_{struct}:C_{active} * (1-\%CO2_{struct:active})\)
    ...
    \end{center}
        ...
        \begin{itemize}
            \item BG\textsubscript{struct}:C\textsubscript{active loss} = below ground structural C being lost as CO\textsubscript{2} during  decomposition into active C (kg ha\textsuperscript{-1})
            \item BG\textsubscript{struct}:C\textsubscript{active act} = below ground structural C decomposed to active C after accounting for CO\textsubscript{2} loss (kg ha\textsuperscript{-1})
        \end{itemize}
        \vspace{0.5cm}
...
\(BG_{struct}:C_{slow\ act}=BG_{struct}:C_{slow} * (1-\%CO2_{struct:slow})\)
    ...
    \end{center}
        ...
        \begin{itemize}
            \item BG\textsubscript{struct}:C\textsubscript{slow loss} = below ground structural C being lost as CO\textsubscript{2} during  decomposition into active C (kg ha\textsuperscript{-1})
            \item BG\textsubscript{struct}:C\textsubscript{slow act} = below ground structural C decomposed to active C after accounting for CO\textsubscript{2} loss (kg ha\textsuperscript{-1})
        \end{itemize}
        \vspace{0.5cm}

The following equations describe the transformation of C among the active, slow, and passive pools, as well as CO2 losses from these pools.
        \vspace{0.5cm}
...
\(Es=0.85-0.68 * silt:clay_\%	\)
    ...
    \begin{center}
\(C_{active:slow}=C_{active_{decomp}} * (1-Es-0.004)\)
\vspace{0.1cm}

\(C_{active:loss}=C_{active_{decomp}} *Es\)
    ...
    \end{center}
        ...
        \begin{itemize}
            \item C\textsubscript{active:slow} = active C decomposed into slow C (kg h\textsuperscript{-1})
    ...
    \begin{center}
\(C_{active}:C_{passive}=C_{active_{decomp}} * 0.004	\)
    ...
    \begin{center}
\(C_{slow:active}=C_{slow_{decomp}} * (1-\%CO2 :C_{slow\ loss}-\%C_{slow:passive})\)
\vspace{0.1cm}

\(C_{slow\ loss}=C_{slow\ decomp} * \%CO2:C_{slow\ loss}\)
\vspace{0.1cm}

\(C_{slow\ passive}=C_{slow\ decomp} * \%C_{slow\ passive}\)
    \end{center}
        \vspace{0.5cm} 
        ...
        \begin{itemize}
            \item C\textsubscript{slow:active} = slow C decomposed into active C (kg ha\textsuperscript{-1})
            \item \%CO\textsubscript{2}:C\textsubscript{slow loss| = slow C lost as CO\textsubscript{2} during decomposition (\%)
            \item \%C\textsubscript{slow:passive} = slow C decomposed into passive C (\%)
            \item C\textsubscript{slow:passive} = slow C decomposed into passive C (kg ha\textsuperscript{-1})
            \item C\textsubscript{slow loss} = slow C lost as CO\textsubscript{2} during decomposition (kg ha\textsuperscript{-1})
        \end{itemize}
        \vspace{0.5cm}

    \begin{center}
\(C_{passive\ loss}=C_{passive\ decomp} * \%CO2:C_{passive\ loss}\)
    \end{center}
        \vspace{0.5cm} 
        ...
        \begin{itemize}
            ...
            \item \%CO\textsubscript{2}:C\textsubscript{passive loss| = passive C lost as CO\textsubscript{2} during decomposition (\%)
...
\(C_{active} = AG:C_{active} + BG:+C_{passive}:C_{active}+C_{slow}:C_{active}-C_{active\ decomp}\)
    \end{center}
        \vspace{0.5cm} 
        ...
        \begin{itemize}
            \item AG:C\textsubscript{active} = above ground C decomposed into the active carbon pool (kg ha\textsuperscript{-1})
            \item BG:C\textsubscript{active} = below ground C decomposed into the active carbon pool (kg ha\textsuperscript{-1})
        \end{itemize}
        \vspace{0.5cm}
...
\(C_{CO2\ loss\ decomp}=C_{active\ loss}+C_{slow\ loss}+C_{passive\ loss}\)
    ...
    \end{center}
        \vspace{0.5cm} 
            ...
            {\raggedleft \textbf{[SC.CAR.48]}\par}
            \vspace{0.5cm}
...
\(C_{CO2\ loss}=AG_{CO2\ loss}+BG_{CO2\ loss}+C_{CO2\ loss\ decomp}\)
    ...
    \end{center}
        \vspace{0.5cm} 
            ...
            {\raggedleft \textbf{[SC.CAR.49]}\par}
            \vspace{0.5cm}
...
The maximum potential crop canopy evaporation and transpiration are calculated as a function of the total potential evapotranspiration for the field on that day (see the \hyperref[soilmgt]{Soil documentation}). 
...
\textbf{Where:}
    \begin{itemize}
        \item fr\subscript{PHU} is the fraction of potential heat units.
