...
substrate (\(1 - NDF_{frac} - CP_{frac} - ash_{frac}\)); \(DML_{4A}\) is the diffusion-driven loss at
the exposed feedout face (zero if packed density is at or near the theoretical maximum); and
\(DML_{4B}\) is a fixed feed-bunk exposure loss, computed unconditionally regardless of \(DML_{4A}\).
Both terms scale with a fixed respiration-rate term (\(MUTAU\)) that is itself a function of dry-matter
content, water activity, and a fixed feeding-time temperature (18\textdegree C) — see
\texttt{RUFAS/biophysical/feed\_storage/silage.py}'s \texttt{calculate\_feed\_out\_loss} for the full
term-by-term translation, cited to \texttt{Silostg.for:1029-1104} (\texttt{FEEDOUT}).
\vspace{0.2cm}
...
For `Bunker`/`Pile` storage, currently-stored crops are grouped into vertical sections before Feed-out
is computed, since a bunker or pile is not fed out one crop at a time:
\vspace{0.2cm}
...
10 days' worth of feed-out at the storage's current rate. Each section's composition (dry matter
fraction, NDF, crude protein, ash) is the mass-weighted average of the crops grouped into it, and its
crop-type flag matches its first crop's. One Feed-out loss fraction (\(DML_4\), \textbf{[FS.SIL.16]})
is computed per section and applied uniformly to every crop within it.
