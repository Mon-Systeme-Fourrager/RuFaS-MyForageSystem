...
def swath_moisture_content(
    ...
    drying_rate_per_hour : float
        Drying-rate constant DR, 1/h (see `dry_bulb_drying_rate`).
    elapsed_hours : float
...
def rain_leaf_loss_fraction(rainfall_in: float) -> float:
    ...
    float
        ...
        orchestrator (`simulate_field_curing`), not by this function.
...
def rain_leaching_loss_fraction(rainfall_in: float, ndf_pct: float) -> float:
    ...
    float
        ...
        `apply_leaching_quality_shift`'s Eq.11.
...
def _day_respiration_loss_fraction(weather: CurrentDayConditions, avg_moisture_wet_basis_fraction: float) -> float:
    ...
    """
    day_temp_f = celsius_to_fahrenheit(weather.max_air_temperature)
    night_temp_f = celsius_to_fahrenheit(weather.min_air_temperature)
    return respiration_dm_loss_fraction(avg_moisture_wet_basis_fraction, day_temp_f) + respiration_dm_loss_fraction(
        avg_moisture_wet_basis_fraction, night_temp_f
    )
...
def _day_rain_losses(
    ...
    tuple[float, float]
        ...
        the whole wilt window (see `simulate_field_curing`'s cap-scope
        note).

    """
    if weather.rainfall <= 0.0:
        return 0.0, 0.0
    rainfall_in = mm_to_inches(weather.rainfall)
    remaining_cap = max(0.0, fcc.FIELD_CURING_RAIN_LEAF_LOSS_CAP_FRACTION - cumulative_rain_leaf_loss_fraction)
    capped_rain_leaf_loss = min(rain_leaf_loss_fraction(rainfall_in), remaining_cap)
    leaching_loss = rain_leaching_loss_fraction(rainfall_in, ndf_pct)
...
def simulate_field_curing(
    ...
    **Rain-leaf cap scope**: DAFOSYM's Eq.9 states its 15% cap as "max 15%
    ...
    for day_index, weather in enumerate(daily_weather):
        ...
        respiration_loss = _day_respiration_loss_fraction(weather, current_moisture)
        rain_leaf_loss, leaching_loss = _day_rain_losses(weather, current_ndf, cumulative_rain_leaf_loss_fraction)
        ...
        if weather.rainfall > 0.0:
            current_cp, current_ndf = apply_leaching_quality_shift(current_cp, current_ndf, leaching_loss)

        drying_rate = _day_drying_rate(weather, swath_density, soil_moisture_at_mowing, is_mowing_day)
        current_moisture = swath_moisture_content(current_moisture, drying_rate, float(GeneralConstants.HOURS_PER_DAY))
