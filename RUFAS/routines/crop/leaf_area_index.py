...
#
def update_all(crop, time):
    l1, l2 = calculate_shape_coefficients(crop)
    calc_fr_LAI_max(crop, time, l1, l2)
    results = calculate_LAI_actual(crop,time)
...
def calculate_shape_coefficients(crop):
    ...
    l1 = log(l1_part1) + l2 * crop.fr_PHU_1
...
# in section 1.D.2
#
def calc_fr_LAI_max(crop, time, l1, l2):
...
def calculate_LAI_actual(crop, time):
    inGrowingPeriod =  crop.planting_date <= time.day <= crop.harvest_date
    crop.prev_LAI_actual = crop.LAI_actual
    ...
    if not inGrowingPeriod:
        ...
        dLAI_max = crop.LAI_actual - crop.prev_LAI_actual
        dLAI_actual = dLAI_max * sqrt(crop.gamma_reg)
