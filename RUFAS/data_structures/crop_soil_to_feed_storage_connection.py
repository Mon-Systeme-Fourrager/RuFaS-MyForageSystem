...
"""
Initial silage temperature by crop type at time of ensiling (degrees C). Alfalfa/haylage value is
Silostg.for's own hardcoded initialization (line 259); the non-alfalfa value is not present in the
read Fortran line ranges (set in the separate CSSILO subroutine) and is instead taken from
Buckmaster, Rotz & Muck (1989), p.1149 "Validation" section, which states this same 8 C assumption
for corn silage where fill temperature was unavailable.

"""
INITIAL_FILL_TEMPERATURE_ALFALFA_C = 18.0
INITIAL_FILL_TEMPERATURE_NON_ALFALFA_C = 8.0
...
class HarvestedCrop:
    ...
    dry_matter_mass : float
        The mass of dry matter in the crop. This is calculated from the fresh mass and dry matter percentage (kg).
    ...
    total_sensible_heat_generated : float
        ...
        stored in a ``Hay`` instance (or one of its child classes), and is only calculated at the time of storage.
    temperature : float
        Current silage temperature in degrees C. Initialized by crop type at storage time and
        subsequently updated only by the Preseal phase's self-heating calculation.
    ...
    def __post_init__(self) -> None:
        """
        Performs post-initialization setup and derived attribute calculations to finalize
        initialization of the ``HarvestedCrop`` instance
        """
    ...
    def estimate_maximum_effluent(self) -> float:
        ...
        -------
        float
            Estimated maximum amount of mass that will flow out of this crop (kg).
    ...
    def _calculate_bale_density(self) -> float:
        ...
        -------
        float
            Bale density of this crop (kg / m^3).
    ...
    def _calculate_total_sensible_heat_generated(self) -> float:
        ...
        -------
        float
            Total sensible heat generated by this crop (kJ / kg).
